let PlayEr = {
    ...
    "play": function(){
        let url = [], contextmenu = ConFig['config']['mp']["contextmenu"].split('#');
        ...
        },mp = ConFig['config']['mp'],u =PlayEr.ad.uic(ConFig['url']),t = PlayEr.type(u);
        playData["src"] = u;
        if(mp["autoplay"] === '1'){
        ...
        if(mp["autoOrientaion"] === '0'){
            playData["autoOrientaion"] = false;
        }
        if(mp["showMiniProgress"] === '0'){
            playData["showMiniProgress"] = false;
        }
        if(mp["dragSpotShape"] === '1'){
            playData["dragSpotShape"] = 'square';
        }
        playData["themeColor"] = ConFig['config']['gx']['color'];
        playData["closeControlsTimer"] = mp["closeControlsTimer"];
        playData["volume"] = mp["volume"];
        playData["lang"] = mp["lang"];
        if(t === 'flv') {
        ...
        PlayEr.void = new MuiPlayer(playData);
        PlayEr.on = document.getElementById("myVideo");
        $("#player .dddd").remove(); /* 修改地方*/
    },
    "full":function(){
    ...
    "load" : function(){
        ...
        box.prepend('<div class="lock-box"></div><div class="pause-ad" style="display:none"><a href="'+ConFig['config']['ads']['pause']['link']+'" target="_blank"><img alt="" src="'+ConFig['config']['ads']['pause']['img']+'">\n' +
            '</a><span class="ad-txt">'+lg['ad-name']+'</span><span class="ad-off">'+lg['ad-close']+'</span></div><div class="ec-danMa text"><div class="ec-danMa-item ec-danMa-item--demo"></div></div><div class="ec-subtitle"></div><div class="header ease flex between"><div class="player-title"></div><div class="flex qoe-normal" style="display:none"><div class="kui-time"></div><div class="batteryShape"><div class="level"><div class="percentage"></div></div></div></div></div>' +
        ...
        $(".right-part").prepend('<button class="player-btn keyboard-input footer-control list-bnt">'+lg['list']+'</button>' +
            '<button class="player-btn keyboard-input footer-control dm-bnt" tooltip="'+lg['dm-label']+'">'+PlayEr.svg.dm+'</button>' +
            '<button class="player-btn keyboard-input footer-control content-bnt" tooltip="'+lg['zm-off']+'">'+PlayEr.svg.content+'</button>');
        $(".left-part").append('<div class="dm-box flex dm-wap"><div class="dm-box-left flex"><div class="dm-box-cc" data-id="0"></div><div class="dm-box-set"></div><div class="dm-set-box ec-box"><div id="dm_n1" class="dm-set-list ds-set-show">\n' +
            ...
            '<button class="dm-send" data-balloon="'+lg['dm-send']+'" data-balloon-pos="up">'+lg['dm-send']+'</button></div></div>');
        ...
        PlayEr.tas();
        //播放进度记录与删除
        PlayEr.on.ontimeupdate = function(){
            let time = PlayEr.on.currentTime;
            PlayEr.cookie.Set("time_" + ConFig['id2'],time,7,ConFig['config']['rm']);
        };
        PlayEr.on.addEventListener("ended",function(){
            PlayEr.cookie.Del("time_" + ConFig['id2'],ConFig['config']['rm']);
        });
        PlayEr.void.on('controls-toggle', function (t) {
            if(t.show === true){
                box.addClass('rap-show');
            }else{
                box.removeClass('rap-show');
            }
        });
        let $el=document,_header = $el.querySelector(".header"),_broadside = $el.querySelector(".broadside")
            ,_list = $el.querySelector(".player-list"),_off = $el.querySelector(".player-list-off")
            ,_lock = $el.querySelector(".lock-box");
        _header.addEventListener("dblclick", function (e) {
            e.stopPropagation();
        });
        _header.addEventListener("click", function (e) {
            e.stopPropagation();
        });
        _header.addEventListener("touchmove", function (e) {
            e.stopPropagation();
        });
        _broadside.addEventListener("dblclick", function (e) {
            e.stopPropagation();
        });
        _broadside.addEventListener("click", function (e) {
            e.stopPropagation();
        });
        _broadside.addEventListener("touchmove", function (e) {
            e.stopPropagation();
        });
        _list.addEventListener("dblclick", function (e) {
            e.stopPropagation();
        });
        ...
        _off.addEventListener("dblclick", function (e) {
            e.stopPropagation();
        });
        _off.addEventListener("click", function (e) {
            e.stopPropagation();
        });
        _off.addEventListener("touchmove", function (e) {
            e.stopPropagation();
        });
        _lock.addEventListener("dblclick", function (e) {
            e.stopPropagation();
        });
        _lock.addEventListener("click", function (e) {
            e.stopPropagation();
        });
        _lock.addEventListener("touchmove", function (e) {
            e.stopPropagation();
        });
    },
    "ad":{
        "Pause" :function(){
            if(ConFig['config']['ads']['pause']['state'] === '1'){
                let ad= $(".pause-ad");
                PlayEr.on.addEventListener("pause",function(){
                    ad.show();
                });
                PlayEr.on.addEventListener("play",function(){
                    ad.hide();
                });
                $(".pause-ad .ad-off").click(function(){
                    ad.hide();
                })
                let $el=document,_ad = $el.querySelector(".pause-ad");
                _ad.addEventListener("dblclick", function (e) {
                    e.stopPropagation();
                });
                _ad.addEventListener("click", function (e) {
                    e.stopPropagation();
                });
                _ad.addEventListener("touchmove", function (e) {
                    e.stopPropagation();
                });
            }
        },
    ...
    "header":{
        ...
        "logo":function(){
            $(".player-wrapper").prepend('<div class="player-logo"><img alt="logo" src="'+ConFig["config"]["gx"]["logo"]+'" /></div>');
        },
        "logoCss":function(){
    ...
    "danMu": {
        ...
        "Init": function () {
            ...
            PlayEr.on.addEventListener("seeking",function(){
                PlayEr.danMu.seek()
            });
            PlayEr.on.addEventListener('play', function () {
                box.addClass("dm-action");
            });
            PlayEr.on.addEventListener('pause', function () {
                box.removeClass("dm-action");
            });
        },
        "api":function(){
        ...
        "frame": function () {
            if (this.dan.length && !PlayEr.on.paused && this.showing) {
                let item = this.dan[this.danIndex];
                const dan = [];
                while (item && PlayEr.on.currentTime > parseFloat(item.time)) {
                    dan.push(item);
        ...
        "_danAnimation": function (position) {
            ...
            const isFullScreen = !!PlayEr.full();
            const animations = {
        ...
        "seek": function () {
            ...
            for (let i = 0; i < this.dan.length; i++) {
                if (this.dan[i].time >= PlayEr.on.currentTime) {
                    this.danIndex = i;
                    break;
                }
        ...
        "content": function () {
            ...
            $(".dm-bnt").click(function(){
                box.addClass("wap-dm-input-show");

                $(".wap-dm-input-show .player-list-off").click(function(){
                    box.removeClass("wap-dm-input-show");
                });

            });


            $(".dplayer-controller,.dm-box-cc").click(function(){
    ...
    "list":{
        ...
        "next":function(){
            if(PlayEr.empty(ConFig["MesData"]["next"]) === false && ConFig["config"]['next_bnt'] === '1'){
                let h = '<button class="dplayer-icon ec-next">'+PlayEr.svg.next+'</button>';
                $(".dplayer-play-icon").after(h);
        ...
        if(ConFig["config"]['lock'] === '1'){
            ...
            h.click(function(){
                if(Number(h.data('id')) === 1){
                    ...
                    box.addClass("lock-show").removeClass("rap-show");
                }else{
                    h.html(PlayEr.svg.open).data('id','1');
                    box.removeClass("lock-show").addClass("rap-show");
                }
            });
        }
    ...
    "LoadAnimation":function(){
        ...
        if(wait === '0'){
            ...
            PlayEr.on.addEventListener("loadedmetadata",function(){
                $("#link3").text(lg['rm5']);
                setTimeout(function() {
                    PlayEr.on.pause();
                    if (b !== '00:00' && b !== 'NaN:NaN') {
                    ...
                    let time = setTimeout(function() {
                        $("#loading").hide();
                        if(ConFig['config']['mp']["autoplay"] === '1'){
                            PlayEr.on.play();
                        }
                    }, (wait*1000));
